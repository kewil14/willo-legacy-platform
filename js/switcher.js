...
jQuery(document).ready(function(){
    ...
    if (jQuery.cookie) {
        if(!jQuery.isEmptyObject( jQuery.cookie() )) {
            deleteAllCookiesHTML = '<span class="rt-icon2-delete-outline" data-toggle="tooltip" data-placement="top" title="Effacer tous les styles (Page sera actualisée)"></span>';
        }
    }

    var switcherHTML = '<div id="switcher">';
    switcherHTML +=    '<div class="color_switcher_header"><h4>Selecteur de Style</h4>';
    switcherHTML +=    		'<span class="rt-icon2-brush"></span>';
    switcherHTML +=    		deleteAllCookiesHTML;
    switcherHTML +=    '</div>';
    switcherHTML +=    '<div id="switcher_accent_color">';
    switcherHTML +=    		'<p>Couleur:</p>';
    ...
    switcherHTML +=    		'<p>Affichage:</p>';
    switcherHTML +=    		'<ul id="switcher-version" class="list-inline">';
    switcherHTML +=        		'<li class="active">';
    switcherHTML +=            		'<a href="#" class="light">Claire</a>';
    switcherHTML +=        		'</li>';
    switcherHTML +=        		'<li>';
    switcherHTML +=            		'<a href="#" class="dark">Sombre</a>';
    switcherHTML +=        		'</li>';
    switcherHTML +=    		'</ul>';
    switcherHTML +=    '</div>';
    switcherHTML +=    '<div id="switcher_layout">';
    switcherHTML +=    		'<p>Style:</p>';
    switcherHTML +=    		'<div class="checkbox checkbox-slider--b-flat">';
    switcherHTML +=    		    '<label>';
    switcherHTML +=    		        '<input type="checkbox" id="layout"> <span>Carré</span>';
    switcherHTML +=    		    '</label>';
    switcherHTML +=    		'</div>';
    switcherHTML +=    		'<div class="checkbox checkbox-slider--b-flat">';
    switcherHTML +=    		    '<label id="boxed_margin" class="hidden">';
    switcherHTML +=    		        '<input type="checkbox"> <span>Margination verticale</span>';
    switcherHTML +=    		    '</label>';
    switcherHTML +=    		'</div>';
    switcherHTML +=		'</div>';
    switcherHTML +=    '<div id="switcher_patterns">';
    switcherHTML +=    		'<p class="for-toggle hidden">Modèles en carré</p>';
